...
uploaded_file = st.file_uploader(
    "Загрузите CSV с временным рядом (Обязательным условием является наличие колонкок 'Order Date' и 'Sales')" \
    " Колонка 'Sales' подразумевает target - ваши продажи",
    type=["csv"]
)
...
if uploaded_file:
    ...
    if not {"Order Date", "Sales"}.issubset(df.columns):
        ...
        # --- Прогноз ---
        prediction_points = len(test) + 30  # прогноз на тест + 30 недель вперёд
        ...
        st.markdown("""
        ##### MAE (Mean Absolute Error) — средняя абсолютная ошибка, показывает насколько в среднем прогноз отклоняется от реальных значений. Чем меньше, тем точнее прогноз.  
        ...
        r2_train  = r2_score(train['y'], train_pred['yhat'])
        mae_test = mean_absolute_error(test['y'], test_pred['yhat'])
        r2_test  = r2_score(test['y'], test_pred['yhat'])


        st.write(f"**Train →** MAE: {mae_train:.2f}, R²: {r2_train:.2f}")
        st.write(f"**Test →** MAE: {mae_test:.2f}, R²: {r2_test:.2f}")
        ...
        ##### Прогноз на 30 недель вперёд: красная линия продолжает ряд на 30 недель после последней доступной даты. Вы видите, как модель ожидает изменяться продажи в будущем на основе предыдущих закономерностей.
        ##### Интервал прогноза (светло-красная область): показывает возможное отклонение прогноза, т.е. диапазон, в котором с высокой вероятностью будут находиться реальные значения.
        """)
